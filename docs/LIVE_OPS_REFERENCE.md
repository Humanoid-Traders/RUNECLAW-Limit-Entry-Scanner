...
| multislot cascade (2026-07-13 probe) | **KILLED** (all-windows rule) | Cascading qualified survivors into open slots vs live one-entry-per-cycle: ~60% more trades at ~45% less per-trade edge; PF down in EVERY window (2.28→1.83 / 3.08→2.05 / 3.34→1.93), net −15.1/−30.7pt at 35/42d, added breakouts dilute worst (win% 77→61), loss-pause fires 66→107 (loss clusters). **One-idea-per-cycle IS a quality filter** (~15–30pt); the runner-ups lost the pooling for a reason. `max_concurrent` being unreachable is a ceiling, not a defect — fifth confirmation of the selectivity family. |
...
| resting-limit condition management (v0.9.41 probes) | **All three KILLED** | Regime-flip cancel −11/−17/−17pt (the dip that fills and the regime flip are the same local extreme — cancelling surrenders the reversion); re-qualification decisive kill (a second min_score raise in disguise); VWAP re-price inert at 0.5 ATR and marginally worse at 0.2. **The exit trio (fill / 3% chase / 4h expiry) is complete** — expiry + fresh re-placement already re-anchors everything. |
